...
function* compileModule(mod, opts) {
  const { dependenciesMap, dest, paths } = opts
  ...
  // paths => [ '/project/root/components' ]
  for (const loadPath of paths) {
    const loadDir = path.dirname(loadPath)
    if (rootPath.startsWith(loadDir) || realPath.startsWith(loadDir)) {
      rootPath = loadDir
    }
  }
...
class Cache {
   ...
   * @param {string} opts.paths    The source paths
   */
  constructor(opts) {
    const { dest, encoding, paths } = opts
  ...
  * removeAll(names) {
    return new Promise((resolve, reject) => {
      ...
      rimraf(dest, { glob: names ? `{${names.join(',')}}` : false }, function(err) {
        if (err) reject(err)
        else resolve()
      })
    })
