...
function compileAll(opts) {
  ...
  var families = [].concat(opts.families || '*')
  ...
  if (/node_modules/.test(base)) {
    return Promise.all([
      _compileAll(base, path.join(families, '*.js'), dest),
      _compileAll(base, path.join(families, '!(node_modules|test)/**/*.js'), dest)
    ])
      .then(function() {
        return _mergeAll(base, families, dest)
      })
  }
  else {
    return _compileAll(base, path.join(families, '**/*.js'), dest)
  }
}
