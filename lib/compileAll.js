 ...
 */
function _process(id, ast, sourceRoot, opts) {
  const { mangle } = Object.assign({ mangle: true }, opts)
  /* eslint-disable camelcase */
  const compressor = new UglifyJS.Compressor({ screw_ie8: false })
...
function* compileModule(id, opts) {
  ...
  const dest = opts.dest && path.resolve(root, opts.dest)
  const result = _process(id, toplevel, opts.sourceRoot, { mangle: opts.mangle })
