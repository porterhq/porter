...
var _parseAlias = require('./parseAlias')
...
function compileAll(opts) {
  ...
  if (!match) {
    return _compileTree({ cwd: cwd, base: base, dest: dest })
  }
...
var RE_COMPONENT_ALIAS = /-[0-9a-f]{8}$/

function _breakAlias(alias) {
  var parts = alias.split('/')
  var version

  while (parts.length > 0) {
    var part = parts.shift()
    if (semver.valid(part)) {
      version = part
      break
    }
  }
...
function _compileTree(opts) {
  var base = opts.base
  var dest = opts.dest
  var aliasMap = _parseAlias(opts)
  var promises = []

  for (var name in aliasMap) {
    var alias = aliasMap[name]

    if (RE_COMPONENT_ALIAS.test(alias)) {
      promises.push(
        _compile({
          base: base,
          fpath: path.join(base, name + '.js'),
          dest: dest
        })
      )
    } else if (alias) {
      var obj = _breakAlias(alias)

      promises.push(
        _compileModule({
          base: path.join(opts.cwd, 'node_modules'),
          name: name,
          main: obj.main + '.js',
          version: obj.version,
          dest: dest
