...
function* _bundle(main, opts) {
  ...
  function* append(id, dependencies, factory) {
    ...
    let babelrcPath = isBundlingComponent
      ? yield findBabelrc(fpath || path.join(paths[0], mod.entry), { root })
      : yield findBabelrc(fpath, { root: paths[0] })
...
function* compileModule(id, opts) {
  ...
  const { toplevel, sourceMaps, moduleIds } = yield* _bundle(id, {
