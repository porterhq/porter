...
function* resolveModule(mod, opts) {
  ...
  const pkgRoot = yield closest(root, mod.name)
  ...
  function* resolveDependency(entry, context) {
    const [fpath, aliased] = entry.endsWith('/')
      ? yield findComponent(`${entry}index.js`, [context])
      : yield findComponent(`${entry}.js`, [context])
...
function* parseMap(opts) {
  ...
  function* resolveComponent(result, component) {
