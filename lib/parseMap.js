...
function* resolveModule(mod, opts) {
  const { root, dependenciesMap, parent } = opts
  const pkgRoot = opts.pkgRoot || (yield closest(root, mod.name))
  ...
  const result = Object.assign({
    ...
    version: pkg.version,
    alias: {}
  }, findModule({ name: mod.name, version: pkg.version }, dependenciesMap))
  const { alias, dependencies } = result
  const resolved = opts.resolved || {}

  // https://github.com/erzu/oceanify/issues/1
  ...
  function* resolveDependency(entry, context) {
    const [fpath, aliased] = yield findComponent(`${entry}.js`, [context])
    let content
    ...
    for (const dep of deps) {
      if (dep[0] == '.') {
        yield* resolveDependency(dep, path.dirname(fpath))
        continue
      }
      ...
      if ((pkg.dependencies && name in pkg.dependencies) ||
        ...
        dependencies[name] = yield* resolveModule({ name, entry: depEntry }, {
  ...
  yield* resolveDependency(mod.entry, pkgRoot)
  return result
}
...
function* parseMap(opts) {
  ...
  function* resolveComponent(result, meta) {
    for (const dep of meta.dependencies) {
      const fullname = dep[0] == '.'
        ? path.join(path.dirname(meta.id), dep)
        : dep

      const [fpath, aliased] = yield* findComponent(`${fullname}.js`, paths)
      if (fpath) {
      ...
      if (name in result || name == pkg.name || RE_URI.test(name)) continue
      ...
      if (version) {
        result[name] = yield* resolveModule({ name, entry }, {
  ...
  for (const currentPath of paths) {
    ...
    const components = yield entries.map(function* (entry) {
