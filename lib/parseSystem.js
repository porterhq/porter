...
function parseModules(dependenciesMap) {
  ...
  function alias(name, data) {
    ...
    const version = versions[data.version] || (versions[data.version] = {})
    ...
    if (data.dependencies && Object.keys(data.dependencies).length > 0) {
      version.dependencies = Object.assign(flat(data.dependencies), version.dependencies)
    }

    if (data.alias && Object.keys(data.alias).length > 0)  {
      version.alias = Object.assign({}, data.alias, version.alias)
    }
