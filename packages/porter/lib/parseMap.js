...
async function resolveModule(mod, opts) {
  ...
  if (!pkgRoot) throw new Error(`Unable to locate module '${mod.name}' while traversing ${root}`)
  ...
  const [mainPath, mainAlias] = await findEntry(mod.entry, pkgRoot)
  if (!mainPath) throw new Error(`Unable to find entry '${mod.entry}' of '${pkg.name}'`)
  Object.assign(result.alias, mainAlias)
      ...
      if (pkg.peerDependencies && name in pkg.peerDependencies) {
        if (name in parent.dependencies) {
          ...
          dependencies[name] = await resolveModule({ name, entry: depEntry }, {
            root, dependenciesMap, parent: result, resolved
          })
        }
      }
      else if ((pkg.dependencies && name in pkg.dependencies) ||
