...
export default class JsModule extends Module {
  matchImport(code: string) {
    ...
    // FIXME
    // find dynamic imports generated by babel plugin, such as import.meta.glob()
    // should be handled in the transform process itself
    for (let i = imports.length - 1; i >= 0; i--) {
      const specifier = imports[i];
      if (code.includes(`Promise.resolve().then(() => _interopRequireWildcard(require("${specifier}")))`)) {
